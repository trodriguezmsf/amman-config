...
FROM
  ...
  INNER JOIN (SELECT
                e.patient_id,
                surgical_diagnosis,
                CONCAT_WS(' ', side, site) AS `side_site`
              FROM encounter e
                INNER JOIN (SELECT
                              v.visit_id,
                              v.patient_id
                ...
                INNER JOIN (SELECT
                              CONCAT_WS(' , ', surgical_diagnosis.diagnosis_coded,
                                        surgical_diagnosis.diagnosis_non_coded) AS `surgical_diagnosis`,
                              surgical_diagnosis.side,
                              surgical_diagnosis.site,
                               ...
                               FROM obs
                                 ...
                                 INNER JOIN concept_name surgical_diagnosis
                                   ON surgical_diagnosis.concept_id = diagnosis.concept_id
                                      AND surgical_diagnosis.name = 'Surgical Diagnosis Section'
                                 ...
                                 INNER JOIN concept_name diagnosis_cn
                                   ON diagnosis_cn.concept_name_type = 'FULLY_SPECIFIED'
                                      ...
                                      AND diagnosis_cn.name = 'Surgical Diagnosis Data'
                                 INNER JOIN concept_name site_of_diagnosis
                                   ON site_of_diagnosis.concept_name_type = 'FULLY_SPECIFIED'
                                      AND site_of_diagnosis.voided IS FALSE
                                      AND site_of_diagnosis.name = 'DIA, Site'
                                 INNER JOIN concept_name side_of_diagnosis_cn
                                   ON side_of_diagnosis_cn.concept_name_type = 'FULLY_SPECIFIED'
                                      AND side_of_diagnosis_cn.voided IS FALSE
                                      AND side_of_diagnosis_cn.name = 'DIA, Side'
                                 ...
                                 LEFT OUTER JOIN obs side_of_surgical_diagnosis
                                   ON side_of_surgical_diagnosis.obs_group_id =
                                      diagnosis.obs_id
                                 ...
                                 LEFT OUTER JOIN concept_name side_coded_answer_cn
                                   ON side_coded_answer_cn.concept_id = side_of_surgical_diagnosis.value_coded
