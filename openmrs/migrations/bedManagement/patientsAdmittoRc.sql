DELETE FROM global_property WHERE property = 'emrapi.sqlSearch.patientsAdmittoRc';
SELECT uuid() INTO @uuid;
INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
VALUES ('emrapi.sqlSearch.patientsAdmittoRc',
"SELECT DISTINCT
...
WHERE v.date_stopped IS NULL AND cn.name = 'Admit to RC' AND p.person_id NOT IN (SELECT patient_id
                                                                                   FROM bed_patient_assignment_map bpam
...
ORDER BY o.obs_datetime;",'Patients Admit to RC',@uuid);