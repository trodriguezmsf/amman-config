...
WHERE v.date_stopped IS NULL AND cn.name = 'Admit to RC' AND v.visit_id NOT IN (SELECT visit_id
                                                                                  FROM encounter ie
...
GROUP BY pi.identifier;",'Patients Admit to RC',@uuid);