DELETE FROM global_property WHERE property = 'emrapi.sqlSearch.admitttokahramana';
SELECT uuid() INTO @uuid;
INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
VALUES ('emrapi.sqlSearch.admitttokahramana',
"SELECT DISTINCT
...
WHERE v.date_stopped IS NULL AND cn.name = 'Admit to Kahramana' AND p.person_id NOT IN (SELECT patient_id
                                                                                   FROM bed_patient_assignment_map bpam
...
ORDER BY o.obs_datetime;",'Patients Admit to Kahramana',@uuid);