...
"SELECT
  ...
  CAST(DATE_FORMAT(COALESCE(nearestFutureAppointment.start_datetime,latestPastAppointment.start_datetime), '%Y-%m-%d') AS CHAR) AS 'Date of Surgery',
  COALESCE(nearestFutureAppointment.surgery_name,latestPastAppointment.surgery_name)                       AS `Surgery`,
