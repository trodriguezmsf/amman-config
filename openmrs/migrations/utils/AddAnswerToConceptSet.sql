CREATE PROCEDURE add_answer_to_concept_set (IN parent_concept_name_proc VARCHAR(255),
                             IN concept_answer_name_proc VARCHAR(255),
                             is_set BOOLEAN)
BEGIN
 DECLARE is_set_val TINYINT(1);
 DECLARE user_id_proc INT;
 DECLARE parent_concept_id_proc INT;
 ...
 SELECT concept_id INTO parent_concept_id_proc FROM concept_name
 WHERE name = parent_concept_name_proc and concept_name_type = "FULLY_SPECIFIED";

 SELECT count(distinct concept_id) into @concept_count from concept_set where concept_set = parent_concept_id_proc and concept_id = concept_answer_id_proc;
