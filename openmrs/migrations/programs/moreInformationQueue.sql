...
"
SELECT  `Date of Presentation` ,  `identifier`,name AS PATIENT_LISTING_QUEUES_HEADER_NAME , uuid , `Nationality` ,`Name of MLO`,`Specialty`,`Outcomes for 1st stage surgical validation`,`Outcomes for 1st stage Anaesthesia validation`,
...
WHERE (`Date of Presentation` IS NOT NULL) AND (`Outcomes for 1st stage surgical validation` ='More Information' OR `Outcomes for 1st stage Anaesthesia validation` = 'Need complementary investigation');",'Patients under more information / complementary investigation needed',@uuid);