 DELETE FROM global_property where property = 'emrapi.sqlSearch.moreInformation';
 select uuid() into @uuid;
 INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
 VALUES ('emrapi.sqlSearch.moreInformation',
"SELECT
  ...
  abc.Outcomes_Anaesthesia_validation as 'Outcomes for 1st Stage Anaesthesia validation',
  CONCAT_WS(', ',abc.surgical_assessment_medical_info, abc.anaesthesia_assessment_medical_info) AS `Type of Medical Information Needed for Next Submission`
FROM patient_identifier pi
  ...
  Left join  (select o.person_id,
                ...
                GROUP_CONCAT(DISTINCT (IF(obs_fscn.name = 'FSTG, Outcomes for 1st stage Anaesthesia validation' AND latest_encounter.person_id IS NOT NULL, COALESCE(coded_fscn.name, coded_scn.name), NULL)) ) AS 'Outcomes_Anaesthesia_validation',
                GROUP_CONCAT(DISTINCT (IF(obs_fscn.name = 'FSTG, Type of medical information needed for next submission' AND latest_encounter.person_id IS NOT NULL, o.value_text, NULL)) ) AS 'surgical_assessment_medical_info',
                GROUP_CONCAT(DISTINCT (IF(obs_fscn.name = 'FST, Type of medical information needed for next submission' AND latest_encounter.person_id IS NOT NULL, o.value_text, NULL)) ) AS 'anaesthesia_assessment_medical_info'
              from encounter e
...
WHERE abc.Date_Presentation is not null AND (abc.Outcomes_surgical_validation ='More Information' OR abc.Outcomes_Anaesthesia_validation = 'Need complementary investigation')
group by pi.patient_id;",'Patients under more information / complementary investigation needed',@uuid);
