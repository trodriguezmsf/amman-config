 DELETE FROM global_property where property = 'emrapi.sqlSearch.recentStoppedPrescriptions';
 select uuid() into @uuid;
 INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
 VALUES ('emrapi.sqlSearch.recentStoppedPrescriptions',
"SELECT
  ...
  DATE_FORMAT(medications.stopDate,'%d/%m/%Y') AS 'Stop Date',
  personData.uuid,
...
FROM
  ...
  INNER JOIN (
               SELECT
                 ...
                 IF(orders.date_stopped IS NOT NULL, orders.date_stopped, orders.auto_expire_date)    AS 'stopDate'
                 FROM patient p
                 ...
                 JOIN orders ON orders.patient_id = pp.patient_id AND orders.encounter_id = e.encounter_id AND
                                        orders.voided IS FALSE 
                                    AND((orders.date_stopped IS NOT NULL
                                    AND TIMESTAMPDIFF(HOUR, orders.date_stopped, NOW()) <= 72)
                                    OR (orders.date_stopped IS NULL AND TIMESTAMPDIFF(HOUR, orders.auto_expire_date, NOW()) > 0 AND TIMESTAMPDIFF(HOUR, orders.auto_expire_date, NOW()) <= 72))
                 ...
                 LEFT JOIN orders stopped_order ON stopped_order.patient_id = pp.patient_id AND stopped_order.voided = 0 AND
                                    (stopped_order.order_action = 'NEW' OR stopped_order.order_action = 'REVISE')
...
ORDER BY medications.stopDate DESC;", 'Recently Closed Prescriptions',@uuid);
