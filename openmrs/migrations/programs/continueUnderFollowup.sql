...
"SELECT IF(`Date Created at DCSF` IS NULL, `Date Of Presentation at Followup`,IF((`Date Created at DOPF` > `Date Created at DCSF`),`Date Of Presentation at Followup` ,`Date Of Presentation at Surgeon Followup`)) AS `Date Of Presentation`,
  pi.identifier,
...
FROM
  ...
  LEFT JOIN person_address paddress ON pi.patient_id = paddress.person_id AND paddress.voided IS FALSE
  LEFT JOIN (
              SELECT pa.person_id,
                IF(pat.name = 'phoneNumber1', pa.value, NULL) AS 'phone1' from
                person_attribute pa
                Inner JOIN person_attribute_type pat ON pat.person_attribute_type_id = pa.person_attribute_type_id AND pat.name = 'phoneNumber1' AND pat.retired IS FALSE where pa.voided IS FALSE
            )phoneNumbers1 ON phoneNumbers1.person_id = pi.patient_id
  LEFT JOIN (
              SELECT pa.person_id,
                IF(pat.name = 'phoneNumber2', pa.value, NULL) AS 'phone2' from
                person_attribute pa
                inner JOIN person_attribute_type pat ON pat.person_attribute_type_id = pa.person_attribute_type_id AND pat.name = 'phoneNumber2' AND pat.retired IS FALSE where pa.voided IS FALSE
            )phoneNumbers2 ON phoneNumbers2.person_id = pi.patient_id
  LEFT JOIN (
              SELECT pa.person_id,
                IF(pat.name = 'phoneNumber3', pa.value, NULL) AS 'phone3' from
                person_attribute pa
                INNER JOIN person_attribute_type pat ON pat.person_attribute_type_id = pa.person_attribute_type_id AND pat.name = 'phoneNumber3' AND pat.retired IS FALSE where pa.voided IS FALSE
            )phoneNumbers3 ON phoneNumbers3.person_id = pi.patient_id
  LEFT JOIN (SELECT
               ...
               ) AS 'Date Of Presentation at Followup',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('FUP, Date of presentation at Followup') AND
                   latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.date_created, '%d-%m-%Y %T'), NULL))
               ) AS 'Date Created at DOPF',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('SFP, Date recorded') AND
                   latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.value_datetime, '%d/%m/%Y'), NULL))
               ) AS 'Date Of Presentation at Surgeon Followup',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('SFP, Date recorded') AND
                   latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.date_created, '%d-%m-%Y %T'), NULL))
               ) AS 'Date Created at DCSF',
               GROUP_CONCAT(DISTINCT (IF(obs_fscn.name = 'FUP, Outcomes for follow-up surgical validation' AND
                                         ...
                                         NULL))
               )                   AS 'outcomeFollowupSurgicalValidation',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('FUP, Time for next medical follow-up') AND
                   latest_encounter.person_id IS NOT NULL,
                   COALESCE(coded_fscn.name, coded_scn.name), NULL)) )                   AS 'Time for next medical follow-up',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('FUP, Time for next medical follow-up') AND
                   latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.date_created, '%d-%m-%Y %T'), NULL))
               ) AS 'Date Created at TFNMF',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('SFP,  MLO visit at:') AND
                   latest_encounter.person_id IS NOT NULL,
                   COALESCE(coded_fscn.name, coded_scn.name), NULL)) )                   AS 'Time for next medical follow-up at Surgeon Followup',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name IN ('SFP,  MLO visit at:') AND
                   latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.date_created, '%d-%m-%Y %T'), NULL))
               ) AS 'Date Created at MLOV',
               GROUP_CONCAT(DISTINCT (IF(
                   obs_fscn.name = 'FUP, Date of next medical follow-up' AND latest_encounter.person_id IS NOT NULL,
                   DATE_FORMAT(o.value_datetime, '%d/%m/%Y'), NULL))
               )                   AS 'Date of next medical follow-up',
               GROUP_CONCAT(DISTINCT (IF(
            ...
            ) followupInfo ON followupInfo.person_id = pi.patient_id
  LEFT JOIN (SELECT
               pp.patient_id,
               SUBSTRING_INDEX(
             ...
             FROM patient_program pp
               ...
               INNER JOIN concept_name cn
                 ON pws.concept_id = cn.concept_id AND concept_name_type = 'FULLY_SPECIFIED' AND
                    cn.voided IS FALSE
             GROUP BY pp.patient_id) program_state ON pi.patient_id = program_state.patient_id
  LEFT JOIN (
              SELECT
                o.person_id,
  ...
  LEFT JOIN (SELECT
    ...
    ON latest_visit.patient_id = pi.patient_id
  LEFT JOIN (select
               o.person_id,
            ...
            )followuptypevalue ON followuptypevalue.person_id = pi.patient_id
  LEFT JOIN (select
               o.person_id,
             ...
             FROM obs o
               ...
               INNER JOIN(
                           ...
                           GROUP BY o.person_id)latest_obsdatetime ON latest_obsdatetime.obs_datecreated = o.date_created AND latest_obsdatetime.person_id = o.person_id
                                                                      AND latest_obsdatetime.concept_id = o.concept_id GROUP BY latest_obsdatetime.person_id
            )surgeontypevalue ON surgeontypevalue.person_id = pi.patient_id
WHERE (
