 DELETE FROM global_property where property = 'emrapi.sqlSearch.postponed';
 select uuid() into @uuid;
 INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
 VALUES ('emrapi.sqlSearch.postponed',
"SELECT  `Date of Presentation` ,  `identifier`,name AS PATIENT_LISTING_QUEUES_HEADER_NAME , uuid , `Nationality` ,`Name of MLO`,`Specialty`,`Outcomes for 1st stage surgical validation`,`Postpone reason`,`Comments About Postpone Reason`,`Medical file to be submitted again by`
FROM (SELECT
...
WHERE ((`Date of Presentation` IS NOT NULL) AND (`Outcomes for 1st stage surgical validation` = 'Postponed')) OR `FV Outcome` = 'Further Evaluation'",'Patients under postponed',@uuid);