SELECT
  o.person_id,
  GROUP_CONCAT(if(qcvn.concept_full_name = 'FSTG, Date received', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))                                 AS `dateOfFileRecieved`,
  GROUP_CONCAT(if(qcvn.concept_full_name = 'FSTG, Date of presentation at 1st stage', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))             AS `dateOfPresentation`,
  ...
  GROUP_CONCAT(if(qcvn.concept_full_name = 'ONN, Date recorded', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))                                  AS `dateRecorderedForONN`,
  GROUP_CONCAT(if(qcvn.concept_full_name = 'Documents, Image Upload', 'Yes', null))                                                                AS `xRayUploaded`,
  GROUP_CONCAT(if(qcvn.concept_full_name = 'Documents, Date', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))                                     AS `xRayUploadedDate`,
  ...
  GROUP_CONCAT(if(qcvn.concept_full_name = 'AIA, date of consultation', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))                           AS `aiaDateOfConsultation`,
  GROUP_CONCAT(if(qcvn.concept_full_name = 'SAP, Date of consultation', DATE_FORMAT(o.value_datetime,'%c/%e/%y'), null))                           AS `sapDateOfConsultation`
FROM obs o
  INNER JOIN concept_view qcvn ON qcvn.concept_id = o.concept_id AND
                                 qcvn.concept_full_name IN ('FSTG, Outcomes for 1st stage surgical validation',
                                              ...
                                              'SAP, Date of consultation') AND qcvn.retired IS FALSE AND o.voided IS FALSE
